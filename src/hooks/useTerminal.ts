...
export function useTerminal(pane: Pane): TerminalController {
  ...
  useEffect(() => {
    ...
    const spawnConnection = () => {
      // Saved sessions need the vault key to decrypt their password/key. If it's locked
      // (master-password mode), prompt for the master password before connecting. Local
      // shells never touch the vault, so they skip this.
      const gate = pane.protocol === 'local' ? Promise.resolve(true) : ensureVaultUnlocked()
      gate.then((unlocked) => {
        if (isDisposed) return
        if (!unlocked) {
          term.writeln(`\r\n\x1b[33mVault is locked. Enter your master password to connect.\x1b[0m`)
          term.writeln(`\x1b[90m[ Press Ctrl+R to try again ]\x1b[0m`)
          setPaneState(pane.id, 'error', 'Vault locked')
          return
        }
        doSpawn()
      })
